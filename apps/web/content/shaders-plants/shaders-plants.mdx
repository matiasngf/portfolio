...
Then, we can create the component that will first load the branch texture and then create the mesh using the `getBranchMesh` function.
...
I needed to create a mesh that grows along a set of points. To achieve this, I used a shaderMaterial that takes a cylinder geometry and a path as a uniform. Then, I used those uniforms to position the vertices of the cylinder along the path.
